...
template <class Executor = decltype(&DefaultPrsExecutor)>
  requires requires(Executor e, const std::string &cmd) {
    { e(cmd) } -> std::convertible_to<int>;
  }
class PrsAction {
public:
  PrsAction(DateRange range, std::string owner,
            Executor exec = DefaultPrsExecutor)
...
private:
  ...
  std::string BuildCommand() const {
    return std::format("gh search prs --owner={}"
                       " --json "
...
PrsAction(DateRange, std::string, Executor) -> PrsAction<Executor>;
